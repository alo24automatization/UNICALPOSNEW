...
module.exports.getReport = async (req, res) => {
  try {
    ...
    let reports = {
      ...
      discounts: {
        discounts: discounts.reduce((summ, discount) => summ + discount.discount, 0),
        discountsuzs: discounts.reduce((summ, discount) => summ + discount.discountuzs, 0),
    ...
    //* ------------------------------- CLIENT BALANCE -------------------------------
    const clientBalanceAggregation = await ClientBalanceTransactions.aggregate([
      {
        $match: {
          client: { $exists: true },
          createdAt: {
    ...
    const createBalance = () => ({ cash: 0, card: 0, transfer: 0, total: 0 });
    const clientBalanceTotals = {
      credit: createBalance(),
      debit: createBalance(),
    };

    clientBalanceAggregation.forEach((balance) => {
      const target = clientBalanceTotals[balance._id];
      if (target) {
        target.cash = balance.cash || 0;
        target.card = balance.card || 0;
        target.transfer = balance.transfer || 0;
        target.total = target.cash + target.card + target.transfer;
      }
    });
    reports.clientsbalance = clientBalanceTotals;
    //! ------------------------------- CLIENT BALANCE -------------------------------
    ...
    sales.map((sale) => {
      ...
      sale.products.map((product) => {
        ...
        incomingpriceuzs += roundUzs(
          product.pieces *
            (product.isMetr
              ? product.product.metrIncPriceOfProduct
    ...
    reports.income.income = roundUsd(
      Number(reports.sale.sale) - Number(incomingprice) - Number(reports.discounts.discounts),
    );
    ...
    const reducecount = (arr, el) => arr.reduce((prev, item) => prev + (item[el] || 0), 0);
    ...
    reports.expenses.expenses = expenses.reduce((prev, expense) => prev + expense.sum, 0);
    reports.expenses.expensesuzs = expenses.reduce((prev, expense) => prev + expense.sumuzs, 0);
    // Get expenses report functions  START
...
module.exports.getSales = async (req, res) => {
  try {
    const { market, currentPage, countPage, startDate, endDate, search } = req.body;
...
module.exports.getProfitData = async (req, res) => {
  try {
    ...
    const saleconnector = await DailySaleConnector.find({
      ...
      .populate('discount', 'discount discountuzs totalprice totalpriceuzs procient');
      ...
      .map((sale) => {
        ...
        const totalincomingpriceuzs = sale.products.reduce(
          (prev, item) =>
            ...
            item.pieces *
              (item.isMetr
                ? item.product.metrIncPriceOfProduct
                : (item.price && item.price.incomingpriceuzs) ||
                  (item.product && item.product.price.incomingpriceuzs) ||
                  0),
          0,
        );
        const totalprice = sale.products.reduce((summ, product) => summ + product.totalprice, 0);
...
module.exports.getPayment = async (req, res) => {
  try {
    const { market, currentPage, countPage, startDate, endDate, search, type } = req.body;
      ...
      .then((connectors) =>
        connectors.filter((connector) =>
          search.client && search.id
            ? connector.saleconnector.client && connector.saleconnector.id === search.id
    ...
    for (const payment of allpayments) {
      respayments.push({
        ...
        client:
          payment.saleconnector && payment.saleconnector.client && payment.saleconnector.client,
        cash: payment.cash,
    ...
    for (const payment of allAgentPayments) {
      totalSum = payment?.packman?.payments.reduce((prev, item) => prev + item.paymentuzs, 0) || 0;
    ...
    for (const client of clients) {
      ...
      if (Math.round((totalpriceuzs - paymentuzs - discountuzs) * 1) / 1 > 0) {
        ...
        for (const sale of sales) {
          ...
          if (Math.round((totalpriceuzs - paymentuzs - discountuzs) * 1) / 1 > 0) {
            const debtComment =
              sale.debts.length > 0 ? sale.debts[sale.debts.length - 1].comment : '';
            const debtId = sale.debts.length > 0 ? sale.debts[sale.debts.length - 1]._id : '';
            const payEndDate =
              sale.debts.length > 0 ? sale.debts[sale.debts.length - 1].pay_end_date : '';
            const returnObj = {
              _id: sale._id,
              id: sale.id,
              createdAt: sale.createdAt,
...
module.exports.getDiscountsReport = async (req, res) => {
  try {
    ...
    const reportdiscount = discountsreport.splice(currentPage * countPage, countPage);
...
module.exports.getBackProducts = async (req, res) => {
  try {
    ...
    const dailyconnector = await DailySaleConnector.find({
      ...
      .populate({
        path: 'products',
        select: 'totalprice totalpriceuzs pieces price unitprice unitpriceuzs product',
        populate: {
          path: 'product',
          select: 'productdata',
          populate: {
  ...
  try {
    ...
    products.map((product) => {
      ...
      productsreport.totalprice +=
        (Math.round(Number(product.price.incomingprice) * 1000) / 1000) * Math.round(product.total);
      productsreport.totalpriceuzs +=
        (Math.round(Number(product.price.incomingpriceuzs) * 1) / 1) * Math.round(product.total);
    });
